...
int main(int argc, char**argv)
{
    ...
    cout << "SL:" << endl
    << desiredState[xarmjoint1].uff << endl
    << desiredState[xarmjoint2].uff << endl
    << desiredState[xarmjoint3].uff << endl
    << desiredState[xarmjoint4].uff << endl
    << desiredState[xarmjoint5].uff << endl
    << desiredState[xarmjoint6].uff << endl
        ;
   return 1;
   //*/
...
static void matlabLog(int numOfTests, int* iterations, double* tests, const std::string& subject) {
    std::string fileName = "asArm_" + subject + "_speed_test_data.m";
    ofstream out(fileName.c_str());
    out << "asarm_test.robot       = 'asArm';" << std::endl;
    out << "asarm_test.description = 'test of the speed of the calculation of: " << subject << "';" << std::endl;
    out << "asarm_test.software    = 'SL';" << std::endl;
    ...
    out << "asarm_test.date = '" << timeStr << "';" << std::endl;

    out << "asarm_test.iterations = [";
    for(int t=0; t<numOfTests; t++) {
        out << " " << iterations[t];
    }
    out << "];" << endl;
    out << "asarm_test.times = [";
    for(int t=0; t<numOfTests; t++) {
        out << " " << tests[t];
    }
    out << "];" << endl;
